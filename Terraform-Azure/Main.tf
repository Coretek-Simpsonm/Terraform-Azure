...
resource "azurerm_resource_group" "msimpson" {
...
resource "random_id" "random_id" {
  keepers = {
    ...
    resource_group = azurerm_resource_group.msimpson.name
  }

  byte_length = 8
}
# Create a virtual network
resource "azurerm_virtual_network" "msimpson-vnet" {
  name                = "msimpson-vnet"
...
resource "azurerm_subnet" "msimpson-subnet" {
  ...
  resource_group_name  = azurerm_resource_group.msimpson.name
  virtual_network_name = azurerm_virtual_network.msimpson-vnet.name
...
resource "azurerm_linux_virtual_machine" "msimpson-linuxVM" {
  name                            = "msimpson-linuxVM"
  resource_group_name             = azurerm_resource_group.msimpson.name
  location                        = azurerm_resource_group.msimpson.location
  size                            = "Standard_B1ms"
  admin_username                  = "setupadmin"
  admin_password                  = azurerm_key_vault_secret.msimpson-LinuxKV.value
  network_interface_ids           = [azurerm_network_interface.msimpson-nic.id]
  computer_name                   = "msimpson-linuxVM"
...
output "public_ip_address" {
  value = "${azurerm_linux_virtual_machine.msimpson-linuxVM.name}: ${data.azurerm_public_ip.msimpson-ipdata.ip_address}"
}